...
For per-project dev credentials, use `direnv` with an `.envrc` file. The CLI reads standard environment variables and does not load `.env` files.
...
Every feature slice uses tests first: RED, then implementation, then review.

Never write implementation before tests exist and are failing.

## CLI Design Rules

- Resource-first commands: `katalogue system get <id>`, not verb-first or API-mirror.
...
- Errors go to stderr with what happened and what to do.
...
Cover each slice with happy path, auth failure, API error, empty results, and missing config.

Mock is patched at `katalogue_cli.cli.common.KatalogueClient`, not at the import site in each command file.

SDK tests go in `packages/katalogue-sdk/tests/`; CLI tests go in `packages/katalogue-cli/tests/`.
`katalogue-sdk` has no Click dependency. Any Click import belongs in `katalogue-cli`.
...
- Use Pydantic for all data models. Prefer `BaseModel` over `@dataclass`.
- Use `SecretStr` for secrets and `field_validator` for validation.
- Do not introduce plain dataclasses when Pydantic is the better fit.
