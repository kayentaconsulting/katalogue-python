...
def list_cmd(ctx: click.Context, fields: list[str] | None, where: list[tuple], fmt: str) -> None:
    """List all glossaries."""
    handle_api_call(ctx, lambda c: c.list_resource("glossary"), fmt, fields=fields, where=where)
...
def get(ctx: click.Context, fields: list[str] | None, glossary_id: str, fmt: str) -> None:
    ...
    handle_api_call(ctx, lambda c: c.get_resource("glossary", glossary_id), fmt, fields=fields)
