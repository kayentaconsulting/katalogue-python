...
def list_cmd(ctx: click.Context, fields: list[str] | None, where: list[tuple], datasource_id: str | None, fmt: str) -> None:
    ...
    if datasource_id:
        handle_api_call(ctx, lambda c: c.list_by_parent("dataset_group", "datasource", datasource_id), fmt, fields=fields, where=where)
    else:
        handle_api_call(ctx, lambda c: c.list_resource("dataset_group"), fmt, fields=fields, where=where)
...
def get(ctx: click.Context, fields: list[str] | None, dataset_group_id: str, fmt: str) -> None:
    ...
    handle_api_call(ctx, lambda c: c.get_resource("dataset_group", dataset_group_id), fmt, fields=fields)
...
def children(ctx: click.Context, fields: list[str] | None, dataset_group_id: str, fmt: str) -> None:
    ...
    handle_api_call(ctx, lambda c: c.list_by_parent("dataset", "dataset_group", dataset_group_id), fmt, fields=fields)
