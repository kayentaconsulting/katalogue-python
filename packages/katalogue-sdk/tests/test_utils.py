"""Tests for katalogue.utils — filter_properties, filter_resultset, sort_resultset, unwrap_list."""

from katalogue.utils import (
    filter_properties,
    filter_resultset,
    sort_resultset,
    unwrap_list,
...
class TestFilterProperties:
    def test_none_properties_returns_data_unchanged(self):
        data = [{"a": 1, "b": 2}]
        assert filter_properties(data, None) is data

    def test_empty_properties_returns_data_unchanged(self):
        data = [{"a": 1, "b": 2}]
        assert filter_properties(data, []) is data
    ...
    def test_list_of_dicts_subset(self):
        ...
        result = filter_properties(data, ["id", "name"])
        assert result == [{"id": 1, "name": "A"}, {"id": 2, "name": "B"}]

    def test_missing_properties_skipped(self):
        data = [{"id": 1}]
        result = filter_properties(data, ["id", "nonexistent"])
        assert result == [{"id": 1}]
    ...
    def test_empty_list_returns_empty(self):
        assert filter_properties([], ["id"]) == []

    def test_non_dict_non_list_returned_unchanged(self):
        assert filter_properties("raw", ["id"]) == "raw"
